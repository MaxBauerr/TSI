...
public class Board {
    ...
    public boolean isBoardEmpty() {
        String initialCellValue = Printer.Format.whiteSafeCell(); // Assuming this returns the initial state of a cell
        for (int x = 0; x < squareSize; x++) {
            for (int y = 0; y < squareSize; y++) {
                if (!board[x][y].equals(initialCellValue)) {
                    return false; // Found a cell that is not in its initial state
                }
            }
        }
        return true; // All cells are in their initial state
    }
    ...
    public boolean isAdjacent(int row, int col, int firstRow, int firstCol) { // Check if current position and mines position are adjacent to each other
        // Check if the positions are the same
        if (row == firstRow && col == firstCol) {
            return false; // The same position is not considered adjacent
        }
        // Check for adjacency
